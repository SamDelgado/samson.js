...
module.exports = function updateHistory(kind, message) {
  ...
  if (kind === "navigate") {
    ...
    var back_safe = this.currentPage ? App.Pages[this.currentPage].backSafe : false;

    // if the currentPage is backSafe, then set it as the previousPage, otherwise set the configured previousPage
    this.previousPage = back_safe ? this.currentPage : App.Pages[this.nextPage].previousPage;
    ...
    this.previousPage = App.Pages[this.currentPage].previousPage;

  } else if (kind === "failed") {
    Samson.log("Router event failed: " + message);
  }
