...
function SamsonComponent(options) {
  ...
  // if the page specified a router 'afterAnimate' event and an onLoad event, then attach them both to 'afterAnimate'
  if (this._router.afterAnimate && this.onLoad) this._router.afterAnimate = function() {self._router.afterAnimate(); self.onLoad()};
  else if (this.onLoad) this._router.afterAnimate = this.onLoad;
